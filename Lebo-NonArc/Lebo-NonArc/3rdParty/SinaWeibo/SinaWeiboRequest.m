...
- (void)handleResponseData:(NSData *)data 
{
    ...
    if ([delegate respondsToSelector:@selector(request:didReceiveRawData:)])
    {
     ...
     NSDictionary *response_dic = [str JSONValue];
    if ([delegate respondsToSelector:@selector(request:didReceiveRawStr:)])
    {
        [delegate request:self didReceiveRawStr:response_dic];
    }
...
- (void)connect:(BOOL)isReload
{
    ...
    [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlString]
                            cachePolicy:isReload ? NSURLRequestReloadIgnoringLocalCacheData : NSURLRequestReturnCacheDataElseLoad
                        timeoutInterval:kSinaWeiboRequestTimeOutInterval];
