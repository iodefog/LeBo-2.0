...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        ...
        NSValue * value = [[arguments objectForKey:@"userInfo"] objectForKey:@"size"];
        UIImage *decodedImage = nil;
        if(value)
            decodedImage = [UIImage decodedImageWithImage:image size:[value CGSizeValue]];
        else
            decodedImage = [UIImage decodedImageWithImage:image];
