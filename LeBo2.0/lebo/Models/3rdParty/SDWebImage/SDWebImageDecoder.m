...
- (void)decodeImageWithInfo:(NSDictionary *)decodeInfo
{
    ...
    NSDictionary * userInfo = [[decodeInfo objectForKey:@"userInfo"] objectForKey:@"userInfo"];
    UIImage *decompressedImage = nil;

    if([userInfo isKindOfClass:[NSNull class]])
    {
        decompressedImage = [UIImage decodedImageWithImage:image];
    }
    else
    {
        NSValue * value = [userInfo objectForKey:@"size"];
        decompressedImage = [UIImage decodedImageWithImage:image size:[value CGSizeValue]];
    }
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image size:(CGSize)size
{
    CGImageAlphaInfo alphaInfo = CGImageGetAlphaInfo(image.CGImage);
    BOOL imageHasAlphaInfo = (alphaInfo != kCGImageAlphaNone &&
                              alphaInfo != kCGImageAlphaNoneSkipFirst &&
    ...
    UIGraphicsBeginImageContextWithOptions(size, !imageHasAlphaInfo, 0);
